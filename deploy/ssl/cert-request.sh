...
if [ -n "$DOMAIN" ] && [ "$DOMAIN" != "localhost" ]; then
	...
	if [ -f ${LETSENCRYPT_DIR:-/etc/letsencrypt}/live/$DOMAIN/privkey.pem ]; then
		...
		cp -f $LETSENCRYPT_DIR/live/$DOMAIN/privkey.pem /usr/share/nginx/certificates/privkey.pem
		cp -f $LETSENCRYPT_DIR/live/$DOMAIN/fullchain.pem /usr/share/nginx/certificates/fullchain.pem
